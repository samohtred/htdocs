...
function db_intelligence(debug_window, cb_click_at, cb_readCookie, cb_writeCookie)
{
//  var this.gui_fields = ['Content', 'Chat', 'Voting'];
  ...
  this.cb_readCookie = cb_readCookie;
  ...
  // others
  this.loadFavorite = db_intelligence_loadFavorite.bind(this);
...
// return the path of the item as Links, Array or String
function db_intelligence_get_tree_item_path(fromItemId, toItemId, callerId, result_type)
{
  ...
  if (currItem != undefined) 
  {
    ...
    var parentIds = this.get_tree_item_parents(toItemId);
                                    // Default : Use first Parent
                                    //  -> for single parents
                                    //  -> if Cookie is missing
    var myParentId = "";
    if (parentIds != undefined)
    {
      myParentId = parentIds[0];
                                    // multiple parents
      if (parentIds.length > 1)
      {
        var parentDefault = this.cb_readCookie(c_PARENT_DEFAULT_COOKIE_NAME_SUFFIX + toItemId);
        if (parentDefault != null)
          myParentId = parentDefault;
      }
    }
                                    // if tree root reached ...
    if (fromItemId == toItemId)
    {
      if (result_type == c_PATH_TYPE_LINK)
        return "<span><a onclick=\"return window.main_dispatcher4input.clicked_at('main_tree', '" + toItemId + "')\">" + this.get_tree_item_field(toItemId, "name") + "</a></span>";
      else 
      if (result_type == c_PATH_TYPE_ARRAY)
        return [toItemId];     
      else
        return ""; // "Alle Themen" is redundant for Favorites but is useful for the Explorer-Style-Path-Link
    }
                                    // otherwise ...
    else
    {
      if (result_type == c_PATH_TYPE_LINK)
      {
        var total_parents = this.get_tree_item_parents(callerId);
                                    // other parent items besides current item available from the 
                                    // point of view of the item which has called this function ?
        if (total_parents.length > 1)
          return this.get_tree_item_path(fromItemId, myParentId, toItemId, c_PATH_TYPE_LINK) + " \\ <span><a onclick=\"return window.main_dispatcher4input.clicked_at('main_tree', '" + toItemId + "')\">" + this.get_tree_item_field(toItemId, "name") + "</a></span> <span><a onclick=\"return window.main_dispatcher4input.clicked_at('select_parents', '" + toItemId + "')\">[...]</a></span>";
        else
          return this.get_tree_item_path(fromItemId, myParentId, toItemId, c_PATH_TYPE_LINK) + " \\ <span><a onclick=\"return window.main_dispatcher4input.clicked_at('main_tree', '" + toItemId + "')\">" + this.get_tree_item_field(toItemId, "name") + "</a></span>";
      }
      else 
      if (result_type == c_PATH_TYPE_ARRAY)
        return this.get_tree_item_path(fromItemId, myParentId, toItemId, c_PATH_TYPE_ARRAY).concat([toItemId]);      
      else
        return this.get_tree_item_path(fromItemId, myParentId, toItemId, c_PATH_TYPE_TEXT) + " \\ " + this.get_tree_item_field(toItemId, "name");        
    }
  }  
  else
  { 
//    alert(c_LANG_WARNING_ITEM_MISSING[global_setup.curr_lang] + toItemId + " !");
  }  
  return undefined;
}


// get favorite path array from Cookie and propagate its parent defaults to 
// respective Cookies
function db_intelligence_loadFavorite(cookieName)
{
  // load path array from Cookie and set all affected parent default Cookies to specific values
  var favPathItemArray = this.cb_readCookie(cookieName); 
  if (favPathItemArray != null)
  {
    for (var i=favPathItemArray.length-1; i>0; i--)
    {
      var myParents = this.get_tree_item_parents(favPathItemArray[i]);
      if (myParents != undefined)
      {
        if (myParents.length > 1)
        {
                                        // write default parent to cookie
          this.cb_writeCookie(c_PARENT_DEFAULT_COOKIE_NAME_SUFFIX + favPathItemArray[i], favPathItemArray[i-1]);
        }
      }
    } 
    return favPathItemArray;
  }
  else
    return null;
}
